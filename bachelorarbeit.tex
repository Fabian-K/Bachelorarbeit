...
\newcommand{\anfRef}[1]{
\ifthenelse {\equal{\pageref{#1}}{\thepage}}    {Anforderung \ref{#1} (A\ref{#1})}{Anforderung \ref{#1} (A\ref{#1} auf \pageref{#1})}
}
...
Durch die Umsetzung dieser beiden  Anforderungen können die Ziele, die im \vref{sec:zielsetzung} festgelegt sind, erreicht werden. Dadurch bieten sich für den Nutzer für die Auswertung und Analyse der Liquiditätssituation einer Bank zusätzliche Möglichkeiten.
...
Die Reihenfolge der Anforderungen spiegelt gleichzeitig auch die Priorität der Umsetzung wieder. Die wichtigste Anforderungen sind  \anfRef{anf:uebertragbarkeit} und \anfRef{anf:korrektheit}. \anfRef{anf:leistung} steht bei der Entwicklung nicht im Mittelpunkt.
...
\useCaseEntry{Vorbedingung}{Erforderliche Zahlungsströme, Liquiditätsszenarien, Liquiditätsgruppen und Laufzeitbänder sind im System vorhanden}
...
Es können Daten aus relationalen Datenbanksystemen, die zum Beispiel in normalisierter Form vorliegen [TODO Glossar?], und aus Data Warehouse-Systeme, die Daten zum Beispiel im Sternschema [TODO ?] ablegen, konsumiert werden. Das Universum abstrahiert diese Daten auf Objekte, die dem Nutzer bereitgestellt werden.\seFootcite{vgl.}{S.49}{Universe.06}
...
Schließlich sind Universen nur mit der Verwendung von BusinessObjects Anwendungen ausgelegt. Das Kriteritum der Übertragbarkeit kann somit nicht garantiert werden.
...
Dabei fällt auf, dass die Umsetzung mit Hilfe von Universen im Vergleich zu der Umsetzung über WebServices deutlich weniger Vorteile bietet bietet. Gleichzeitig bieten die Universen zusätzlich not mehr Nachteile. Unter diesen Nachteilen findet sich auch die ungelöste Frage der Parameterübergabe und die fehlende Übertragbarkeit auf andere Anwendungen, welche als wichtige Anforderung definiert ist.
