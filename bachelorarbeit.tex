...
Im Anschluss darauf wird das Konzept der Zwischenschicht in Form einer Spezifikation entwickelt. Dabei werden nach einer Bestimmung der Ausgangs- und der Zielsituation konkrete Anforderungen an die Zwischenschicht aufgestellt. Genauer wird hier die gewünschte Interaktion mit der Zwischenschicht erleutert.
...
Die eigentliche Umsetzung wird in einem seperaten Kapitel behandelt. Dabei wird der komplette Entwicklungsprozess von der Analyse, über den Entwurf bis zur Implementierung genauer erleutert.
...
In der Arbeit wird vor Allem im Kapitel der Umsetzung die kursive Schreibweise für die Hervorhebung von Klassen, Instanzen und Methoden genutzt. Bei der Verwendung von Zitaten wird nur der Schlüssel der referenzierten Quelle angegeben. Genauere Informationen zu der Quellen können dann im Literaturverzeichniss unter dem Schlüssel abgerufen werden. Steht hinter einem Begriff das Symbol \textsuperscript{GL} ist dieser im Glossar der Arbeit näher erläutert.

\chapter{Grundlagen des Liquiditätsrisikomanagements} \todonum{Sprechendere Überschrift finden, Grundlagen ist nicht schön}
...
\chapter{SAP LRM und BusinessObjects Xcelsius} \todonum{Anderer Titel!}
...
\end{figure} \todonum{Abbildung erstellen}
...
\label{sec:xcelsius}
\subsection{Überblick und Funktionsumfang} \todonum{Richtiger Titel?}
...
\end{figure} \todonum{Einbindung der Abbildung überprüfen}
...
\todonum{Framework erklären?! -> wenn gute erklärung gefunden wird}
